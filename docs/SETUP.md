...
|------|----------|:---:|:---:|:---:|:---:|
| [**Self-Hosted**](#self-hosted-setup) | Full power, always running | $6–12 | ✅ | ✅ | ✅ |
| [**n8n Cloud**](#n8n-cloud-setup) | Quick start, no server management | $24+ | ❌ | ❌ (text only) | ✅ |
...
| AI Model | Model string (e.g., `gemma-4-26b-a4b-it`) | Tells the AI node which model to use |
...
- A 1GB droplet ($6/month) works for the core stack (n8n + Postgres + Caddy) but will OOM-kill if you run both sidecars simultaneously. Start with 1GB if budget is tight, upgrade when you enable JobSpy + CV renderer together.
- Add your SSH key during creation

Other VPS providers (Hetzner, Vultr, Linode) work fine — you just need Docker and Docker Compose.
...
**Email credential (choose one):**
- **Gmail**: Add Credential → **Send Email** → your Gmail + App Password, SSL on, port 465
- **Resend**: Add Credential → **Resend** → paste your Resend API key
...
Go to [n8n.io](https://n8n.io) and create an account. The Starter plan ($24/month) works.
...
If using Google AI Studio / Gemma 4: make sure **Output Content as JSON** is toggled **OFF** in the OpenAI node settings. The system prompts enforce JSON output directly.
